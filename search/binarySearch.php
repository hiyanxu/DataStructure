...
/*
 * 实例问题：如何快速定位出一个IP地址的归属地
 * （1）若IP区间与归属地对应关系不经常更新，我们可以先预处理这12万条数据
 * （2）将IP地址转换为整数，然后按照整数排序。
 * （3）查找当前IP满足“最后一个小于等于当前IP的IP”，获取该IP地址后，判断该IP区间内是否包含当前需定位的IP，若包含则返回，若不包含，则表示未找到
 * */
